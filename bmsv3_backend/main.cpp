...
void init_database() 
{
    ...
    if (movie_count == 0) 
    {
        ...
        const char* seed_sql =
            ...
            "('Solaris Rising', 'solaris rising.png', 'Humanity''s last hope rests in a colossal, solar-powered space station that is orbiting a dying star. A small crew on the station must find a way to reignite the star, or they will be plunged into a cosmic cold darkness, ending the human race for good.', 105, 'PG', 9.1),"
            "('The Alchemist''s Secret', 'the alchemists secret.png', 'An alchemist discovers the legendary secret to creating a new type of element. This new substance is said to have the power to create a new, better world, but a powerful, shadowy organization wants to use its power for destruction. The alchemist must protect his secret at all costs before it falls into the wrong hands.', 105, 'PG', 7.2),"
            "('Warden of the Void', 'warden of the void.png', 'A lone, heavily-armored warrior is the Warden of the Void, a guardian of the universe''s most dangerous prison: a massive, swirling black vortex in deep space. He must face down a malevolent, otherworldly force that is trying to escape from the vortex, using his wits and weaponry to protect the universe from a cosmic threat.', 105, 'PG-13', 8.6),"
            "('Chrono Heist', 'chrono heist.png', 'A master thief in a futuristic city is tasked with stealing a valuable historical artifact from a highly secure museum. He discovers that the artifact is a temporal device, and as he tries to steal it, he finds himself in a high-stakes, time-bending heist where he must navigate holographic displays of historical events and a constantly changing reality to escape and prevent a temporal paradox.', 105, 'PG-13', 8.9),"
            "('Neon Serpent', 'neon serpent.png', 'In a futuristic, cyberpunk city, a mysterious, glowing, neon-colored serpent appears in the city''s rain-soaked streets. As it slithers through the city''s alleyways, it leaves a trail of destruction in its wake. A lone detective must track down the serpent and uncover its origins to prevent it from destroying the city.', 105, 'PG-13', 7.7),"
...
int main() 
{
    ...
    ([]()
    {
        ...
        sqlite3_stmt* stmt;
        const char* sql_select = "SELECT MovieID, Title, PosterURL, Synopsis, DurationMinutes, Rating, Score FROM Movies";
        ...
        if (sqlite3_prepare_v2(db, sql_select, -1, &stmt, 0) == SQLITE_OK) 
        {
            while (sqlite3_step(stmt) == SQLITE_ROW) 
            {
                ...
                movie["score"] = sqlite3_column_double(stmt, 6);
    ...
    CROW_ROUTE(app, "/movies/top-rated").methods("GET"_method)
    ([]()
    {
        json movies_json = json::array();
        sqlite3_stmt* stmt;
        const char* sql_select = "SELECT MovieID, Title, PosterURL, Synopsis, DurationMinutes, Rating, Score FROM Movies ORDER BY Score DESC";
    ...
    CROW_ROUTE(app, "/venues/top-rated").methods("GET"_method)
    ([](){
        json venues_json = json::array();
        sqlite3_stmt* stmt;
        const char* sql_select = "SELECT VenueID, Name, Location, ImageURL, AuditoriumCount, Rating FROM Venues ORDER BY Rating DESC";
